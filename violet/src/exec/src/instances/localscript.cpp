...
LocalScript::LocalScript(const Instance& instance) : Instance(instance) {}

std::string LocalScript::GetBytecode() const {
    if (!IsValid()) return "";

	uintptr_t embedded = ReadFrom<uintptr_t>(offsets::LocalScript::Bytecode);
...
void LocalScript::SetBytecode(const std::string& bytecode, bool revert) const {
    if (!IsValid()) return;
	...
	process->Write<uintptr_t>(embedded + offsets::ByteCode::Pointer, reinterpret_cast<uintptr_t>(new_bytecode_ptr)) || !process->Write<uint64_t>(embedded + offsets::ByteCode::Size, bytecode.size());
}
